...
class Math_model:
    ...
    def create_objective(self) -> None:
        ...
        transport_flow_cost = sum(0.11571952 * self.vars['arc_flow'][node1, node2] * 1e-6 *  self.arc_cost[node1, node2] * self.crf * self.duration
                                        for (node1, node2) in self.a_a) # $M * {0, 1} = $M

        # pipeline_build_cost = sum(self.vars['arc_build_cost'][node1, node2] *  self.arc_cost[node1, node2] * self.crf 
        #                                 for (node1, node2) in self.a_a) # $M * {0, 1} = $M
        
        pipeline_build_cost = sum(self.vars['arc_built'][node1, node2] * 0.431655132 *  self.arc_cost[node1, node2] * self.crf * self.duration
                                        for (node1, node2) in self.a_a) # $M * {0, 1} = $M
    ...
    def extract_costs(self) -> None:
        ...
        for arc in self.soln_arcs.keys():
            tf_cost = 0.11571952 * self.vars['arc_flow'][arc].X * 1e-6 *  self.arc_cost[arc] * self.crf * self.duration
            tb_cost = self.vars['arc_built'][arc].x * 0.431655132 *  self.arc_cost[arc] * self.crf * self.duration
